...
import { create, find, findOne, findOneAndUpdate, updateOne } from "../../DB/database.repository.js";
...
import { emailEvents } from "../../Utils/email/email.events.js";
...
export const signup = async(req,res)=>{
    ...
    // hashing OTP:
    const hashedOTP = await generateHash({plaintext:otp, algorithm: HashEnum.Argon2})
    ...
    const new_user = await create({
        model: UserModel,
        data: [
            {username, email, password: hashedPassword, phone:encryptedPhone, role, confirmEmailOTP: hashedOTP}
        ],
    });

    // send email to user:
    emailEvents.emit("confirmEmail" , {to:email, otp})
...
export const confirmEmail = async(req,res)=>{
    // receive email and otp from req.body:
    const {email, otp} = req.body;

    // check if user email exists and if user has confirmEmailOTP and if user has confirmEmail:
    const existing_user = await findOne({model: UserModel, filter: {email, confirmEmailOTP:{$exists: true}, confirmEmail:{$exists: false}} });
    if(!existing_user) return exception.BadRequestException("user not found or user has not OTP in DB or user has confirmEmail date");

    // compare otp:
    const matchOTP = await compareHash({
        plaintext: otp, // (data) came from req.body
        ciphertext: existing_user.confirmEmailOTP, // (hashed value) came from DB
        algorithm: HashEnum.Argon2,
    });
    if(!matchOTP) return exception.UnauthorizedException("invalid OTP");

    // update existing_user:
    await updateOne({model: UserModel, filter:{email}, update:{confirmEmail: Date.now(), $unset:{confirmEmailOTP:true}}})

    return successResponse({
        res , statusCode: 200 , message: "user confirmed successfully",
    });
}
...
export const login = async(req,res)=>{
    ...
    // check if email exists and if confirmEmail exists:
    const existing_user = await findOne({model: UserModel, filter: { email, confirmEmail: {$exists: true} } });
...
export const forgetPassword = async(req,res)=>{
    ...
    // check // if email exists in my DB // if confirmEmail is true // if provider is system
    const existing_user = await findOneAndUpdate({
        model: UserModel,
        filter: {
            email,
    ...
    // sending mail contains otp to user email
    emailEvents.emit("forgetPassword", {to:email, otp})

    return successResponse({
        res , message: "check your inbox"
    });
}

export const resetPassword = async(req,res)=>{
    // user send email & otp & newPassword & confirmPassword (req.body)
    const {email, otp, newPassword, confirmPassword} = req.body;

    // check if email exists & if provider is system & if confirmEmail is exists & if forgetPasswordOTP is exists
    const existing_user = await findOne({
        model: UserModel,
        filter:{
            email,
    ...
    // check or compare [otp that coming from (req.body)] with [forgetPasswordOTP (in my DB)]
    const isValidOTP = await compareHash({
        plaintext: otp,
        ciphertext: existing_user.forgetPasswordOTP,
        algorithm: HashEnum.Argon2
    });
    if(!isValidOTP) return exception.BadRequestException("Invalid OTP");
    ...
    // update user password (in my DB) // unset forgetPasswordOTP (in my DB)
    await updateOne({
        model: UserModel,
        filter: {email},
        update: {
